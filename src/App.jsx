...
function App() {
  ...
  useEffect(() => {
    // Show splash screen/loader for 600ms
    const timer = setTimeout(() => {
      setLoading(false);
    }, 600);
    
    // Hide splash screen overlay from DOM after it fades out (900ms total)
  ...
  return (
    <>
      <style>
        {`
          ...
          body {
            ...
            margin: 0;
            padding: 0;
            background: linear-gradient(320deg, #3f00e6, #962cff) !important;
            height: 100vh;
            overflow: hidden;
          }
        `}
      ...
      <div 
        className="h-screen w-full flex justify-center items-center overflow-hidden box-border"
        style={{
          background: 'linear-gradient(320deg, #3f00e6, #962cff)',
        }}
      >
        ...
        {splashVisible && (
          <div 
            className={`fixed inset-0 flex flex-col justify-center items-center overflow-hidden z-50 transition-opacity duration-300 ${
